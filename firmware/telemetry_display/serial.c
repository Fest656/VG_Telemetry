...
/* 
 *
 * \ Function definition for the getchar we used
...
int serLineRead(char *buffer, int maxLength) {
    ...
    while (i < (maxLength - 1)) { // Need to save room for null termination
        c = getchar_timeout_us(SERIAL_CHAR_TIMEOUT_US);
        if (c == PICO_ERROR_TIMEOUT) {
            printf("Error: serLineRead timed out.\n");
            return 0;
        }
        if (c == '\n') { // EOL, fill buffer
            buffer[i] = '\0';
    ...
    printf("Error: serLineRead buffer full before newline was found.\n");
    return 0;
}
...
int serDataHandler(GameState *statePtr, const char *buffer) {
    int parsedData = sscanf(buffer, "%d;%d;%d;%d;%d;%d", &statePtr->health, &statePtr->armor, &statePtr->magAmmo, &statePtr->reserveAmmo, &statePtr->killCount, &statePtr->deathCount);
    // Check if sscanf actually parsed the correct number of data elements
    if (parsedData != 6) {
        printf("Error: serDataHandler expected 6 fields, parsed: %d.\n", parsedData);
        return 0;
    }
    return 1;
}

// Printf puts characters in the console which is transmitted over the COM port back to the PC
